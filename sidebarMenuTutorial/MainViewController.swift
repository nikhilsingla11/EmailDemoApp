...
class MainViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: NSOperationQueue(), completionHandler:{
           ...
           if (parseError == nil)
            {
                if let messages : NSArray = json as? NSArray
                {
...
//        var localDict : NSMutableDictionary = messageDict
        var mailDict : NSDictionary!
//        if((localDict.objectForKey(indexPath.row+1)) != nil)
//        {
            mailDict  = self.messageArray[indexPath.row] as NSDictionary
//            mailDict  = localDict.objectForKey(indexPath.row+1) as NSDictionary
            var newLabel = UILabel(frame: CGRectMake(15, 10.0, 600.0, 15.0))
            var newLabel2 = UILabel(frame: CGRectMake(15, 30.0, 600.0, 15.0))
            var newLabel3 = UILabel(frame: CGRectMake(15, 45.0, 600.0, 15.0))
            ...
            var tempArr = mailDict.objectForKey("participants") as NSArray
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        var indexDict : NSDictionary!  = self.messageArray[indexPath.row] as NSDictionary
